...
class CrawlThread(threading.Thread):
    def __init__(self,teskQueue,dataQueue,threadname):
        super(CrawlThread,self).__init__()
        self.teskQueue = teskQueue
        self.dataQueue = dataQueue
        self.threadname = threadname

    def run(self):
        #1.根据url发起请求（http://blog.jobbole.com/all-posts/page/4/）
        while self.teskQueue.empty() is not True:
            print(self.threadname+'正在下载数据')
            page = self.teskQueue.get()
...
class ParseThread(threading.Thread):
    def __init__(self,dataQueue,threadname,lock):
    ...
    def run(self):
        #1.从数据队列中取出响应的结果
        #2.解析数据
        while self.dataQueue.empty() is not True:
...
if __name__ == '__main__':
    ...
    for crawl_name in crawl_thread_names:
        td = CrawlThread(teskQueue,dataQueue,crawl_name)
    ...
    for parse_name in parse_thread_names:
        #构造解析线程
        td = ParseThread(dataQueue,parse_name,lock)
