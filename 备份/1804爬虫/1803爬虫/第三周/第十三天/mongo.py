...
def update_data():
    ...
    print(result)
    #全文档更新只修改一条
    # result = student_col.update({'name':'liyong'},{'name':'美女'})
    # print(result)
    #更新超照到的全部结果修改多条
    # result = student_col.update_many({'name':'美女'},{'$set':{'name':'徐资源'}})
    # print(result)
    #　使用save做更新操作,全文档更新
...
def find_data():
    ...
    #<pymongo.cursor.Cursor object at 0x7f64596da908>
    result = student_col.find({'name':'好人'})
    print(result)
    print([i for i in result])

    result = student_col.find_one_and_delete()
    # result = student_col.find_one_and_replace()
    # result = student_col.find_one_and_update()

    find_one查询时，直接返回一个字典
    result = student_col.find_one({'name':'好人'})
    print(result)
    print(type(result))
