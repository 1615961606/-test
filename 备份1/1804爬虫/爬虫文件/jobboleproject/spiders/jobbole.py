...
class JobboleSpider(scrapy.Spider):
    ...
    def parse(self, response):
        """
        response:请求的响应结果,
        我们一般在这里进行数据的解析,和新的url的提取
        """
        #第一步：获取文章的列表，提取文章的详情url,发起请求
        articles = response.xpath('//div[@id="archive"]/div[@class="post floated-thumb"]')
        for article_note in articles:
            #文章的url地址
            #extract():将数据转换为unicode编码的列表
            # article_url = article_note.xpath('.//a[@class="archive-title"]/@href').extract()[0]
            article_url = article_note.xpath('.//a[@class="archive-title"]/@href').extract_first()
            coverImageUrl = article_note.xpath('.//div[@class="post-thumb"]/a/img/@src').extract_first()
            print(article_url)
        ...
        #获取当前页面，其它分页的url,继续发起请求
        other_pages = response.xpath('//div[@class="navigation margin-20"]/a/@href').extract()
    ...
    def parse_article_detail(self,response):
        ...
        jobbole_item['votenums'] = self.get_num(votenums)
        # 收藏量
